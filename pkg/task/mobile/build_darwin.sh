...
export BUNDLE_DIR="$(realpath "../../../")/build/${SDK}/Grexie Cloud.app"
...
if [ "$SDK" = "macosx" ]; then
  export BUNDLE_DIR="$(realpath "../../../")/build/${SDK}/Grexie Cloud.app/Contents"
  mkdir -p "${BUNDLE_DIR}/"
fi
...
if ! [ "$SDK" = "macosx" ]; then
  ...
  go build -a -o "${BUNDLE_DIR}/cloud" -tags ${NODE_ENV:-development} .
  ...
  go build -a -o "${BUNDLE_DIR}/MacOS/cloud" -tags ${NODE_ENV:-development} .
