...
func newAgent(in isolates.FunctionArgs, https Https) (*AgentBase, error) {
	agent := &AgentBase{
		https: https,
	}
...
func (a *AgentBase) CreateConnection(ctx context.Context, options net.ConnectOptions, connectListener *isolates.Value) (net.Socket, error) {
	...
	if socketv, err := a.https.Net().Connect(ctx, "tcp", fmt.Sprintf("%s:%d", *options.Host, options.Port)); err != nil {
		return nil, err
